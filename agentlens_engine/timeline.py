...
.bar-llm{background:var(--blue)}.bar-tool{background:var(--amber)}.bar-error{background:var(--red)}.bar-memory{background:var(--purple)}
...
.role-system{color:var(--purple)}.role-user{color:var(--blue)}.role-assistant{color:var(--green)}.role-tool{color:var(--amber)}
...
function dotCls(t){return t==='llm_call'?'llm':t==='tool_call'?'tool':t==='error'?'error':t==='memory_snapshot'?'memory':'tool'}
